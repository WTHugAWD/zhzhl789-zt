<template>
	<div class="body">
		<home-navbar back="返回" title="选择课种" setting="" @backFun="backFun" @settingFun="settingFun"></home-navbar>
		<div class="container">
			<!--进度-->
			<div class="stepsList">
			...
			<div class="coach-list">
				<div class="coach-module" v-for="(item,index) in list" :key="index">
					...
					<div class="coach-right">
						<div class="coach-right-info">
							<div class="info-box">
								<router-link :to="'curriculumdetails1?id='+item.Id">
									<div class="info-name">{{item.Name}}</div>
									<div class="time-slot" v-if="item.StartTime!=null && item.EndTime!=null">{{item.StartTime.split("T")[1].split(":")[0]}}:{{item.StartTime.split("T")[1].split(":")[1]}} ~ {{item.EndTime.split("T")[1].split(":")[0]}}:{{item.EndTime.split("T")[1].split(":")[1]}}</div>
									<div class="coach-name van-clearfix">
										<!--<van-icon class="info-icon" name="clock" />
										<span class="info-icon-text">{{item.Name}}课时</span>-->
									</div>
								</router-link>
							</div>
							<div class="coach-btn">
								<router-link :to="'coachdate1?id='+item.Id">
									<span>立即选择</span>
								</router-link>
							</div>
						</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//通过教练获取该教练的课种列表
			GetCurrPageList(){
				getInfo("Order/GetCurrPageList?memberId="+this.$route.query.id+"&classId=''&time=''&pageIndex="+this.pageIndex+"&pageSize="+this.pageSize,{}).then(res=>{
					if(res.data.type==1){
						console.log(res.data)
						this.list = res.data.resultdata.Rows;
					}
				}).catch(err=>{
					console.log(err)
				})
