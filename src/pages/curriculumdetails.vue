<template>
	<div class="body">
		<home-navbar back="返回" title="课程详情" setting="" @backFun="backFun" @settingFun="settingFun"></home-navbar>
		<div class="container">
			<!--进度-->
			<!--<div class="stepsList">
			...
			<div class="details-content">
				<p ref="detailsContent">
					
				</p>
			</div>
		</div>
		<div class="footer-btn">
			<router-link :to="'coachdate?id='+dataInfo.Id">
			...
			GetCurriculumAsync(){
				getInfo('Order/GetCurriculumAsync?id='+this.$route.query.id,{}).then(res=>{
					if(res.data.type==1){
						...
						this.$refs.detailsContent.innerHTML = res.data.resultdata.Content;
					}
				}).catch(err=>{
					console.log(err)
				})
